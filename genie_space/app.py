...
app.layout = html.Div([
    html.Div([
        ...
        dcc.Store(id="insight-trigger-store", data=None), # New store for insight generation flow
...
def call_llm_for_insights(df, prompt=None):
    ...
    """
    # Instruction to be appended to the prompt to control formatting.
    # This is not visible to the user in the default prompt textarea.
    formatting_instruction = "\n\nIMPORTANT: Do not use markdown headers (e.g., '#', '##'). Instead, use bolding for titles (e.g., '**Key Insights**')."

    if prompt is None:
        # Define the default prompt that is visible to the user.
        prompt = (
            "You are a professional data analyst. Given the following table data, provide deep, actionable analysis for\n"
            "1. Key insights and trends.\n"
            "2. Notable patterns\n"
    ...
    # Concatenate the user-facing prompt with the hidden formatting instruction.
    final_prompt = prompt + formatting_instruction
...
# NEW: Callback to trigger insight generation (adds thinking indicator)
@app.callback(
    [Output("chat-messages", "children", allow_duplicate=True),
     Output("query-running-store", "data", allow_duplicate=True),
...
def trigger_insight_generation(n_clicks, prompt_value, table_uuid, current_messages, query_running):
    if not n_clicks or not table_uuid:
        return no_update, no_update, no_update

    # Prevent starting a new insight generation while one is running
    if query_running:
        return no_update, no_update, no_update
    ...
    thinking_indicator = html.Div([
        html.Div([
            html.Span(className="spinner"),
            html.Span("Generating insights...")
        ], className="thinking-indicator")
    ], className="bot-message message")

    updated_messages = current_messages + [thinking_indicator]

    # Prepare trigger for the next callback
    trigger_data = {
        "table_uuid": table_uuid,
        "prompt_value": prompt_value,
    }

    # Set query running to True and trigger the generation
    return updated_messages, True, trigger_data

# MODIFIED: Callback to generate insights and append to chat
@app.callback(
    [Output("chat-messages", "children", allow_duplicate=True),
     Output("query-running-store", "data", allow_duplicate=True),
     Output("chat-history-store", "data", allow_duplicate=True),
     Output("insight-trigger-store", "data", allow_duplicate=True)],  # Clear the trigger
...
def confirm_and_generate_insights(trigger_data, chat_history, current_messages):
    ...
    # Remove the thinking indicator before adding the final message
    ...
    if df is None:
        ...
        ], className="bot-message message", id=f"insight-error-{uuid.uuid4()}")
